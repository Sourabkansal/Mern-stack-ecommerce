...
const signup = () => {
  ...
  const onSubmit = async (data) => {
    console.log(data)
    let response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/signup`, {
      method: "POST",
      headers: {
  ...
  async function sendotpp() {
    ...
    let response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/getotp`,{
         method:"POST",
         headers:{
          "Content-type": "application/json",
         },
    ...
    // const notify = () =>
    //   toast( response.status==400?"email is required!":
    //     response.status==401?"Enter valid Email " :"Otp send sucess");
      ...
      <div className=" mt-5 sm:mt-0 w-lg sm:w-[60%] bg-white rounded-2xl shadow-lg p-6 flex flex-col md:flex-row md:space-x-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-2">
            A brand new day is here. It’s your day to shape. Sign up and get
            started on shopping.
          </p>
          {/* Form */}
          <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
              ...
              <input
                ...
                {...register("username", {
                  ...
                  pattern: {
                    ...
                    message: "use alphabet and number",
                  },
                })}
              />
              {errors.username && (
            ...
            {
               !sendotp &&
               <button type="button" onClick={sendOtp} disabled={IssendingOtp} className="w-full bg-[rgba(254,169,40,0.6)] hover:bg-[rgba(254,169,40,0.6)] text-white py-3 rounded-lg">
                {IssendingOtp ? "Sending Otp...":"Send Otp"}</button>
            ...
            <>
             ...
             <input  placeholder='Example:- 1234' className="mt-1 w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[rgba(254,169,40,0.4)]" 
             {...register("otp",{ required:{message:"Enter OTP "}})}  />
             </div>
            <input
              type="submit"
